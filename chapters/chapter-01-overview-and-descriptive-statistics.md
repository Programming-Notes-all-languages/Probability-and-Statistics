...
**Sensitivity to outliers.** The mean's key weakness: even one outlier (an unusually large or small observation) can greatly affect it. In Example 1.14, $x_{21} = 45.0$ is an obvious outlier:
...
$n = 12$ is even, so the median averages the $n/2 = 6$-th and $(n/2 + 1) = 7$-th ordered values:
...
$\sum x_i = 365.9$, so $\bar{x} = 33.26$. (The deviations sum to .04 rather than exactly 0 — rounding effects.) The numerator of $s^2$ is $S_{xx} = \sum (x_i - \bar{x})^2 = 314.106$, from which
...
It is customary to refer to $s^2$ as being based on $n - 1$ **degrees of freedom (df)**. Although $s^2$ is built from the $n$ quantities $x_1 - \bar{x}, \ldots, x_n - \bar{x}$, these sum to 0, so specifying any $n-1$ of them determines the remaining one. For example, with $n = 4$, if $x_1 - \bar{x} = 8$, $x_2 - \bar{x} = -6$, and $x_3 - \bar{x} = -2$, then automatically $x_4 - \bar{x} = 0$ — only three of the four deviations are freely determined (3 df).
...
The sum is $\sum x_i = 1695$ and the sum of squares is $\sum x_i^2 = (154)^2 + (142)^2 + \cdots + (122)^2 = 222{,}581$, so

$$S_{xx} = \sum x_i^2 - \frac{(\sum x_i)^2}{n} = 222{,}581 - \frac{(1695)^2}{13} = 1579.0769,$$
...
Relevant summary quantities: $\tilde{x} = 92.17$, lower fourth $= 45.64$, upper fourth $= 167.79$, so $f_s = 122.15$; hence $1.5\,f_s = 183.225$ and $3\,f_s = 366.45$. Subtracting $1.5\,f_s$ from the lower fourth gives a negative number and no observation is negative — **no outliers on the low end**. On the high end:
